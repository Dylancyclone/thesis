...
#define MS_BETWEEN_KEYSTROKE 100
...
private:
  ...
  char *temp;
  ENTRY ep;
  struct input_event ev, inject_ev;
  ...
  map<string, int> keymap_reverse = reverse_map(keymap);
    ...
    {
      {
        ...
        inject_ev.code = keymap_reverse[string(1, c)];
        inject_ev.value = 1; // press key
        write(fd, &inject_ev, sizeof(inject_ev));
        inject_ev.value = 0; // unpress key
        write(fd, &inject_ev, sizeof(inject_ev));
      }
    }
  }
...
public:
  KeyLogger(int argc, char **argv)
  {
    if (argc != 2)
    {
      cerr << "usage: " << argv[0] << " event-device - probably /dev/input/event1" << endl;
      exit(1);
    }
  ...
  void log()
  {
    ...
    while (1)
    {
      ...
      string search_result = keymap[ev.code];
      if (!search_result.empty())
      {
        if (ev.value != 0) // Ignore if key is being released
        {
          ...
          {
            // If Enter is pressed, begin the test
            keysInputCounter = 0;
            colorsInputCounter = 0;

            thread type_thread(&KeyLogger::type_string, this);
            type_thread.detach();
          }
          // get time

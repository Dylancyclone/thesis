...
\todoadvisor{How many samples were taken to create the figures? How many times was the test run?}
...
\begin{enumerate}
  ...
  \item The user presses the \enquote{0} key on the client's numpad to synchronize \todoadvisor{Syncronize what?}the two devices' data recording tools.\todoadvisor{How far off are they?}
  \item The user begins by typing on the client's keyboard \todoadvisor{what does slowly mean?}slowly, allowing both computers to record their data.
  \item The user then types on the client's keyboard quickly to ensure that \todoadvisor{Why is there a seperation here?}quick successive inputs are handled correctly and no data is lost or duplicated.
...
\end{figure}
\todoadvisor{Maybe put full figure in the appendix?}

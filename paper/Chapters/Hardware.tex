...
\todoadvisor{Target user or audience of the product should be described more accurately. Since no studies or research has been conducted regarding which group of people are interested in / will use this product, no specific requirements can be derived for the system.
At this point the only source of the requirements is the author of the thesis and hence it should be worded this way. The thesis should initially state that the requirements for the user experience are based on the authors requirements/preferences, to make clear that no other users were involved or a study conducted with other persons.}
...
\todoadvisor{Why are those components not described in the same way as power/USB/etc.?}\todoquestion{The Ethernet port is hooked directly into the CM4 connector with nothing interested except that they are also differential pairs, as described in the HDMI section. I suppose they could be explained fully, but I worry that it wouldn't add any useful information.}\todoadvisor{Explain why it's skimed over}
...
Unfortunately, the component \emph{U2 (MCP73871-2CCI\_ML)}, is described as using the \emph{QFN-20-1EP\_4x4mm} footprint in its datasheet\todoquestion{Does this need to be cited? Can you cite a datasheet like this?}\todoadvisor{Yes}, but actually uses a revised version of the footprint, shown in Figure \ref{fig:FootprintQFN204x4}.

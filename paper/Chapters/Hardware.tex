...
This is because as data is transmitted across the pair of tracks, the two signals need to arrive at their destination at the same time, a time that is determined by the physical distance the signals need to cover.
Since the speed of electricity is constant, the traces may need to wind back and forth so that the lengths positive and negative tracks are equal.
Also note the cutout area at the top of the CM4, whose perimeter is stitched by vias; this is where the CM4 houses it's bluetooth and wifi antennas.

\begin{figure}[t]
  \centering
  \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[width=.5\linewidth]{Figures/kicad/close-ups/cm4-front}
    \caption{The front side of the PCB}
    \label{fig:CM4CircuitFront}
  \end{subfigure}%
  \begin{subfigure}{.5\textwidth}
    \centering
    \includegraphics[width=.5\linewidth]{Figures/kicad/close-ups/cm4-back}
    \caption{The back side of the PCB}
